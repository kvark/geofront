...
- **Presentation** — Quaternius skinned GLBs play Idle / Walk / Punch / Hit / Death via `Engine::set_animation`. Attacks use a telegraph wind-up then strike (Splinter snappy, Mass heavy); hit reactions are deferred to impact; death locks and freezes so wrecks stay down. Tokyo-3 twilight mood: warm sodium street lamps vs cool dusk sky, dense skyscraper canyon around the 8×8 grid, low hero combat camera with impact framing; strikes land with a hit flash, local-light / sodium kick, and a short camera punch. Move tiles and facing drawn on the ground. Directional shadows stay off under lavapipe (skinned mechs).
